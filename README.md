...
- **스켈레톤 로딩**: 모든 페이지에서 콘텐츠 로딩 시 스켈레톤 UI로 부드러운 전환
- **키보드 단축키 도움말**: `?` 키로 전체 단축키 목록을 모달로 확인
- **데이터 초기화**: 푸터에서 localStorage 데이터 리셋 (시드 데이터 재로드)
- **필터 결과 실시간 안내**: aria-live 영역으로 필터 결과 수 변경을 스크린 리더에 자동 안내
- **접근성**: Skip-to-content, ARIA, 키보드 내비게이션, 포커스 트랩, prefers-reduced-motion, aria-live 필터 결과
...
| **확장/아이디어** | 30% | 북마크, 다크모드, 통계차트, 최근본해커톤, 진행률바, 마감알림, 재제출, Ctrl+K검색, 그리드/리스트, CSV내보내기, 공유, 스켈레톤, 스크롤투탑, 토스트, 키보드단축키도움말(?키), 데이터초기화 |
| **완성도** | 25% | ErrorBoundary, try/catch, next/image, 접근성(aria-describedby, aria-invalid, aria-live), 반응형, 키보드 내비게이션, 스켈레톤 로딩, useMemo 최적화 |
