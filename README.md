...
- **글로벌 랭킹**: 전체/월별/연도별 기간 필터, 해커톤별 필터, 컬럼 정렬
...
| **확장/아이디어** | 30% | 북마크, 다크모드, 통계, 진행률바, 마감알림, 재제출, 토스트 |
| **완성도** | 25% | ErrorBoundary, try/catch, 접근성, 반응형, 키보드 내비게이션 |
| **문서화** | 15% | README, 프로젝트 구조, 데이터 구조, 사용자 흐름 설명 |

## 페이지 구조

| 경로 | 페이지 | 설명 |
|---|---|---|
| `/` | 메인 | 통계 대시보드, 마감 임박 알림, 해커톤/팀/랭킹 프리뷰, 북마크 |
...
| `/rankings` | 글로벌 랭킹 | 기간별/해커톤별 필터, 컬럼 정렬 가능 |
...
├── lib/                    # 유틸리티
│   ├── storage.ts          # localStorage CRUD (try/catch 에러 핸들링)
...
- ARIA 속성 (`aria-label`, `aria-required`, `aria-pressed`, `aria-selected`, `aria-live`, `aria-hidden`)
